...
<div class="form-card" x-data="{ photoPreview: null }">
    <form action="{{ route('admin.etudiants.update', $etudiant->id) }}" method="POST" enctype="multipart/form-data">
        @csrf @method('PUT')
        ...
        <div class="form-grid">
            <div class="form-field">
                ...
                <input id="nom" name="nom" type="text" class="form-input" value="{{ old('nom', $etudiant->user->nom) }}" required>
                <x-input-error :messages="$errors->get('nom')" />
            </div>
            <div class="form-field">
                <label class="form-label" for="prenom">Prénom(s)</label>
                <input id="prenom" name="prenom" type="text" class="form-input" value="{{ old('prenom', $etudiant->user->prenom) }}" required>
                <x-input-error :messages="$errors->get('prenom')" />
            </div>
            <div class="form-field">
                <label class="form-label" for="email">Email</label>
                <input id="email" name="email" type="email" class="form-input" value="{{ old('email', $etudiant->user->email) }}" required>
                <x-input-error :messages="$errors->get('email')" />
            </div>
            <div class="form-field">
                <label class="form-label" for="telephone">Téléphone</label>
                <input id="telephone" name="telephone" type="text" class="form-input" value="{{ old('telephone', $etudiant->user->telephone) }}">
        ...
        <div class="form-section">
            ...
            <div class="form-grid">
                <div class="form-field">
                    ...
                    <input id="matricule" name="matricule" type="text" class="form-input" value="{{ old('matricule', $etudiant->matricule) }}" required>
                ...
                <div class="form-field" style="grid-column:1/-1;">
                    ...
                    <select id="filiere" name="filiere" class="form-input select2-field" required>
                        @php $filieres = ['Génie Logiciel','Cybersécurité','Réseaux et Télécommunications','Intelligence Artificielle','Data Science','Informatique de Gestion','Développement Web et Mobile','Systèmes d\'Information']; @endphp
                        @foreach($filieres as $f)
                        <option value="{{ $f }}" {{ old('filiere', $etudiant->filiere) == $f ? 'selected' : '' }}>{{ $f }}</option>
                        @endforeach
                        @if(!in_array(old('filiere',$etudiant->filiere), $filieres) && old('filiere',$etudiant->filiere))
                        <option value="{{ old('filiere',$etudiant->filiere) }}" selected>{{ old('filiere',$etudiant->filiere) }}</option>
                        @endif
                    </select>
