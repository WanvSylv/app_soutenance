<x-app-layout>
    @section('header', 'Modifier la Planification')
    ...
    <div class="max-w-5xl mx-auto animate-fade-in">
        ...
        <div class="glass-card p-10">
            <form action="{{ route('admin.planification.update', $soutenance->id) }}" method="POST" class="space-y-10">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-10">
                    ...
                    <div class="space-y-8">
                        ...
                        <div class="group">
                            ...
                            <textarea id="sujet" name="sujet" rows="4" class="mt-1 block w-full border-[#E0E5F2] bg-[#F4F7FE] text-[#1B254B] focus:border-[#2D60FF] focus:ring-[#2D60FF]/10 rounded-[1.25rem] shadow-sm transition-all duration-300 font-semibold" required placeholder="Saisir le titre complet du mémoire...">{{ old('sujet', $soutenance->sujet) }}</textarea>
                    ...
                    <div class="space-y-8">
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            <div class="group">
                                ...
                                <x-text-input id="date_soutenance" name="date_soutenance" type="date" class="block w-full" :value="old('date_soutenance', $soutenance->date_heure_debut->format('Y-m-d'))" required />
                                <x-input-error class="mt-2" :messages="$errors->get('date_soutenance')" />
                            </div>
                            <div class="group">
                                <x-input-label for="heure_debut" :value="__('Heure de début')" />
                                <x-text-input id="heure_debut" name="heure_debut" type="time" class="block w-full" :value="old('heure_debut', $soutenance->date_heure_debut->format('H:i'))" required />
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
                        <div class="group">
                            ...
                            <select id="president_id" name="president_id" class="mt-1 block w-full border-[#E0E5F2] bg-[#F4F7FE] text-[#1B254B] focus:border-[#2D60FF] focus:ring-[#2D60FF]/10 rounded-[1.25rem] shadow-sm transition-all duration-300 font-semibold" required>
                                ...
                                @foreach($enseignants as $enseignant)
                                    <option value="{{ $enseignant->id }}" {{ (old('president_id', $president ? $president->enseignant_id : '') == $enseignant->id) ? 'selected' : '' }}>
                        ...
                        <div class="group">
                            ...
                            <select id="rapporteur_id" name="rapporteur_id" class="mt-1 block w-full border-[#E0E5F2] bg-[#F4F7FE] text-[#1B254B] focus:border-[#2D60FF] focus:ring-[#2D60FF]/10 rounded-[1.25rem] shadow-sm transition-all duration-300 font-semibold" required>
                                <option value="">Choisir...</option>
                                @foreach($enseignants as $enseignant)
                                    <option value="{{ $enseignant->id }}" {{ (old('rapporteur_id', $rapporteur ? $rapporteur->enseignant_id : '') == $enseignant->id) ? 'selected' : '' }}>
                                        {{ $enseignant->user->nom }} {{ $enseignant->user->prenom }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="group">
                            ...
                            <select id="membre_id" name="membre_id" class="mt-1 block w-full border-[#E0E5F2] bg-[#F4F7FE] text-[#1B254B] focus:border-[#2D60FF] focus:ring-[#2D60FF]/10 rounded-[1.25rem] shadow-sm transition-all duration-300 font-semibold">
                                ...
                                @foreach($enseignants as $enseignant)
                                    <option value="{{ $enseignant->id }}" {{ (old('membre_id', $membre ? $membre->enseignant_id : '') == $enseignant->id) ? 'selected' : '' }}>
                                        {{ $enseignant->user->nom }} {{ $enseignant->user->prenom }}
                                    </option>
                                @endforeach
                            </select>
